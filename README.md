...
This could be a good idea. However, notice in our design that some patients might have both levels of the variable of interest. Thus, the situation does not always become much simpler after pseudo-bulking. Furthermore, for this study, we wanted to retain the individual AOIs for other analyses, like ssGSEA. This was a bit of a Catch-22, to be honest. I opted to keep the individual AOIs, account for pseudo-replication as best I could, and represent the unbalanced structure of the data in the final contrasts being tested.
...
I think that we can debate to no end about the "correct" way to construct the contrasts for testing. The whole point of this analysis though is to identify genes that have differing expression patterns across the groups of interest. Looking at the heatmaps in the manuscripts, I think that we were able to do that; one can see clear differences. Overall, my outlook is this: we have tried hard to be statistically rigorous in our methods and we have thought through how we are representing the data, which has allowed us to identify some real differences, evidenced by visualization.

Our modeling and testing are not perfect. The exact steps taken for the manuscript may not be quite as aligned with best practices as certain alternative modeling strategies. In fact, from now on, I will likely turn to mixed modeling for designs like this one. However, we nonetheless captured some reasonable differences in the data that are biologically interesting and supportable. Overall, I think that the current analysis can stand.

If interested, contact me for discussion. I have done a deep dive into this topic with supplementary plots that I can share. Furthermore, the method of constructing and testing weighted contrasts that I have described above is equivalent to "cell-weighted" marginal means from the `emmeans` package. I made sure to note this in the final bladder manuscript methods section. For reference, see here: https://cran.r-project.org/web/packages/emmeans/vignettes/messy-data.html#weights. 

In the general case, one would do the following with `emmeans`: 

```
# a simple model
mod <- lm(formula = expr ~ condition + patient, data = d)

# emmeans method
EMM <- emmeans::emmeans(object = mod, specs = c("condition"), weights = "cells")
pairs(EMM)
